...
if (isset($_POST['submit'])) {
    if (!empty($_POST['email']) && !empty($_POST['pns'])) {
        $email = mysqli_real_escape_string($conn, $_POST['email']);
        $valid_id = mysqli_real_escape_string($conn, $_POST['pns']);
        
        $query = "SELECT a.email AS email, v.id AS id, v.valid_id AS valid, v.app_id AS app
                  FROM account a
                  INNER JOIN verified v ON a.account_id = v.id
                  WHERE a.email = '$email' AND v.valid_id = '$valid_id'";
                  
        $result = mysqli_query($conn, $query);

        if (mysqli_num_rows($result) > 0) {
            $assoc = mysqli_fetch_assoc($result);
            $_SESSION['reset_email'] = $assoc['email'];
            $_SESSION['reset_id'] = $assoc['id'];
            echo "<script>alert('Congrats you can now change your Passcode')</script>;";
            echo "<script>window.location.href='reset.php?'</script>";
        } else {
            echo "<script>alert('No user found in the database')</script>;";
        }
    } else {
    ...
    <div class="container">
        ...
        <form method="POST" action="<?php $_SERVER['PHP_SELF']; ?>">
            <div class="entryInput">
                ...
                <input type="text" name="pns" placeholder="Enter PNS ID">
            </div>
            <div class="entryInput">
