...
if(isset($_POST['submit'])) {
  if(isset($_POST['email']) && isset($_POST['passcode'])) {
    ...
    if(strlen($email) >= 4 && strlen($passcode) >= 4) {
      $query = "SELECT * FROM account where email ='$email' and passcode ='$passcode'";
      ...
      if($email == $fetch['email'] && $passcode == $fetch['passcode']) {
        ...
        $assocQr = mysqli_fetch_assoc($resQr);
        $uniqueQrCode = $assocQr['valid_id']."-".$assocQr['app_id'];
        $_SESSION['photo'] = $assocQr['photo'];
        echo "<script>localStorage.setItem('qrcode_unique','$uniqueQrCode')</script>";
      ...
      echo "<script>alert('Both Input Must Have Equal or Greater than 4')</script>;";
    }
  }else{
...
<body>
    <div class="container">
        ...
        <form method="POST" action="<?php $_SERVER['PHP_SELF']; ?>">
            ...
            <a id="forget" href="forgot.html">Forget Pasword?</a>
